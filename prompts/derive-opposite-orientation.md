...
彻底清除源模板中的全部文字。`ai_integrated` 只生成“{certificate_title}”并锁定 x=50%；`vector_flat` 或 `vector_effect` 全图不生成文字，标题由收尾脚本绘制。目标为竖版时使用 x 24%–76%、y 9%–18%，正式输出上移约 110 px。最终成品禁止其他文字或伪文字。
