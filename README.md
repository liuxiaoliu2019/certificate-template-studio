...
它既可以从教材封面提炼 Style DNA、锁定人物与动物角色特征并探索证书方向，也可以将现成横版或竖版证书模板高保真重制为横竖双 Master。v1.7 默认采用质量托管：内部比较三套横版，自动提交最佳合格方案，每个方向最多自动修正一次；同时通过阶段上下文、来源哈希缓存和双 Master 换标题减少重复分析、用户干预与上下文消耗。模板转换额外锁定原模板的标题结构：弧形、飘带承载物、字体气质及平面/效果材质不会再被默认直排金字替换。
...
- **模板标题结构锁**：弧形和飘带标题保留原生无字承载物；平面标题继承纯色与描边，禁止未请求的渐变、金属高光和阴影。
